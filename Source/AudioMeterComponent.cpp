...
void AudioMeterComponent::paint (juce::Graphics& g)
{
    ...
    float scaledXL = juce::jmap(static_cast<float>(rmsCurrentValueL), 0.0f, 0.9f, 0.0f, static_cast<float>(getWidth()));
    float scaledXR = juce::jmap(static_cast<float>(rmsCurrentValueR), 0.0f, 0.9f, 0.0f, static_cast<float>(getWidth()));
//    DBG(scaledXL);
//    DBG(scaledXR);
